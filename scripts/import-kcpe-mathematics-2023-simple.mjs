...
/**
 * KCPE Mathematics 2023 Kenya - V2 Import Script (Simple)
 * 
 * This script imports the KCPE Mathematics 2023 exam into the live XamPreps V2 Firestore system.
 * It uses the existing V2 creation functions from the project.
 * 
 * Usage: node scripts/import-kcpe-mathematics-2023-simple.mjs
 */
...
// Import the V2 creation functions - these will be available when the script runs in the project context
let createExam, createSection, createInstructionGroup, createItem, createInteraction, createMarkingRule, createModelAnswerVersion;

async function loadV2Functions() {
  try {
    const v2Module = await import('../src/integrations/firebase/v2-collections.js');
    createExam = v2Module.createExam;
    createSection = v2Module.createSection;
    createInstructionGroup = v2Module.createInstructionGroup;
    createItem = v2Module.createItem;
    createInteraction = v2Module.createInteraction;
    createMarkingRule = v2Module.createMarkingRule;
    createModelAnswerVersion = v2Module.createModelAnswerVersion;
    console.log('✅ V2 functions loaded successfully');
  } catch (error) {
    console.error('❌ Failed to load V2 functions:', error.message);
    throw error;
  }
}
