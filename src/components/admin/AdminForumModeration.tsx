...
import {
  deleteForumCategoryFirebase,
  deleteForumPostFirebase,
  listForumCategoriesFirebase,
  listForumPostsFirebase,
  setForumPostLockedFirebase,
  setForumPostPinnedFirebase,
  upsertForumCategoryFirebase,
...
export default function AdminForumModeration() {
    ...
    try {
      ...
      if (postsRes.ok) {
        const categoryNameMap = new Map(
          (categoriesRes.items || []).map((c) => [c.id, c.name])
        );
        setPosts(
          postsRes.items.map((post) => ({
            ...post,
            category_name: post.category_name || categoryNameMap.get(post.category_id) || 'Unknown',
          }))
        );
      } else {
        setPosts([]);
      ...
      console.error('Failed to fetch forum moderation data', error);
      toast.error('Failed to load forum moderation data');
    }
    setLoading(false);
  };
  ...
  const handleSaveCategory = async () => {
    ...
    try {
      const result = await upsertForumCategoryFirebase({
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    ...
    try {
      const result = await deleteForumCategoryFirebase(categoryId);
      if (!result.ok) {
  ...
  const handleTogglePin = async (post: ForumPost) => {
    try {
      const result = await setForumPostPinnedFirebase(post.id, !post.is_pinned);
      if (!result.ok) {
  ...
  const handleToggleLock = async (post: ForumPost) => {
    try {
      const result = await setForumPostLockedFirebase(post.id, !post.is_locked);
      if (!result.ok) {
        toast.error('Failed to update post');
      } else {
  ...
  const handleDeletePost = async (postId: string) => {
    ...
    try {
      const result = await deleteForumPostFirebase(postId);
      if (!result.ok) {
